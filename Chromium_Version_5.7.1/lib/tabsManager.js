...
CMH.tabsManager.onHeadersReceived = async (requestDetails) => {
  let certificateFormatted = null;
  if (CMH.common.isWebExtTlsApiSupported()) {
    const securityInfo = await chrome.webRequest.getSecurityInfo(requestDetails.requestId, { certificateChain: true })
    if (securityInfo.state === 'secure' || securityInfo.state === 'weak') {
      certificateFormatted = CMH.certificatesManager.formatCertificate(securityInfo.certificates)
    }
  } else if (CMH.native.connected) {
    const certRes = await CMH.native.postMessageAndWaitResponse({ action: 'CHECK', host: new URL(requestDetails.url).host, tabId: requestDetails.tabId })
    if (certRes && certRes.sha256) {
      certificateFormatted = {
        fingerprints: {
          sha256: certRes.sha256.replace(/:/g, '').toUpperCase()
        }
      }
    }
  }
...
chrome.webRequest.onHeadersReceived.addListener(CMH.tabsManager.onHeadersReceivedForIp,
  { urls: ['https://*/*'] },
  ['blocking']
)
