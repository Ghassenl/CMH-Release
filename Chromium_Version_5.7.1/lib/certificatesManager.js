...
CMH.certificatesManager.getCertUrl = async (urlTested, httpHeadMethod = false) => {
  ...
  let cert = null
  let headersDetails = null
  let retry = null
  const controller = new AbortController();

  const listener = async (details) => {
    headersDetails = details
    chrome.webRequest.onHeadersReceived.removeListener(listener)


    if (CMH.common.isWebExtTlsApiSupported()) {
      const securityInfo = await chrome.webRequest.getSecurityInfo(details.requestId, { certificateChain: true })
  ...
  try {
    ...
    if (!CMH.common.isWebExtTlsApiSupported()) {
      const certRes = await CMH.native.postMessageAndWaitResponse({
        action: 'CHECK',
        host: new URL(headersDetails?.url ?? urlTested).host,
        tabId: headersDetails?.tabId ?? -99999
      })
